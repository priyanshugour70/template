...
| File | Purpose |
|------|---------|
| `config.go` | Static configuration via Viper/godotenv — structs for App, Server, Postgres, Redis, SMTP, CORS, Auth, Assets (S3), Developer. Loads `.env` → `.env.{APP_ENV}` → `.env.local`. Production validation enforces JWT secret length and CORS origins. |
...
**Dynamic (DB-based):** Add a typed accessor method to `BizConfigReader` here. Implement it in your `bizconfig.Service` and surface it through a thin admin UI / API.
